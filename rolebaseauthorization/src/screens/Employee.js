...
const Employee = () => {
  ...
  const getData = () => {
    ...
    axios
      .get("https://localhost:7228/api/employee/get")
      .then((result) => {
        setData(result.data);
      })
      .catch((error) => {
  ...
  const handleSave = () => {
    const url = "https://localhost:7228/api/employee/post";
    const data = {
      "name": name,
      "address": address,
  ...
  const handleEdit = (id) => {
    ...
    handleShow();
    axios
      .get(`https://localhost:7228/api/employee/getbyid/${id}`)
      .then((result) => {
  ...
  const handleDelete = (id) => {
    if (window.confirm("Are you sure to delete this data") == true) {
      axios
        .delete(`https://localhost:7228/api/employee/delete/${id}`)
        .then((result) => {
  ...
  const handleUpdate = () => {
    const uRl = `https://localhost:7228/api/employee/update/${editId}`;
    const data = {
      id: editId,
      name: editname,
      address: editaddress,
